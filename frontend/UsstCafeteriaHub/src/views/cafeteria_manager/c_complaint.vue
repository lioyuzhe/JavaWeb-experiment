<template>
  <div>
    ...
    <el-table :data="tableData" stripe :header-cell-style="{ backgroundColor: 'aliceblue', color: '#666' }" @selection-change="handleSelectionChange">
      <el-table-column prop="complaintId" label="序号" width="70" align="center"></el-table-column>
    ...
    <el-dialog title="投诉信息" :visible.sync="fromVisible" width="30%">
      <el-form :model="form" label-width="80px" style="padding-right: 20px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="投诉内容" prop="content">
          <el-input type="textarea" v-model="form.content" placeholder="投诉内容"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    load(pageNum) {
      this.$request.get('/complaints/actions/getComplaintsByCondition', {
        params: {
    ...
    del(complaint) {
      this.$confirm('您确认删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.post('/complaints/actions/deleteComplaint', complaint).then(res => {
          if (res.code === 200) {
    ...
    save() {
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.complaintId ? '/complaints/actions/updateComplaint': '/complaints/actions/addComplaint',
            method: 'POST',
            data: this.form
          }).then(res => {
            if (res.code === 200) {
